...
RSpec.describe List do
	...
	context "# Dieta española" do
		before :each do
			...
			@es_pollo = Node.new(Alimento.new([6,0,3,5.7,7.1], "pollo", 1), nil, nil)
			@es_lenteja = Node.new(Alimento.new([8,25,1,0.4,3.4], "lentejas", 1), nil, nil)
			@es_nuez = Node.new(Alimento.new([6,15,36,0.3,7.9], "nueces", 1), nil, nil)
			@va_camarones = Node.new(Alimento.new([14.5,10,30,18,2], "camarones", 1), nil, nil)
			@va_cerveza = Node.new(Alimento.new([0.5,30,10,0.24, 0.22], "cerveza", 1), nil, nil)
			@ve_leche = Node.new(Alimento.new([12,5,7,3.2,8.9], "leche", 1), nil, nil)
			@ve_huevos = Node.new(Alimento.new([3,5,3,4.2,5.7], "huevos", 1), nil, nil)
			@ve_chocolate = Node.new(Alimento.new([5,30,20,2.3,3.4], "chocolate", 1), nil, nil)
			@veg_cafe = Node.new(Alimento.new([0,18,15,0.4,0.3], "cafe", 1), nil ,nil)
			@veg_tofu = Node.new(Alimento.new([0,22,35,2,2.2], "tofu", 1), nil, nil)
			@veg_lentejas = Node.new(Alimento.new([10,15,5,0.4,3.4], "lentejas", 1), nil, nil)
			@lxc_vaca = Node.new(Alimento.new([20,5,1,50,164], "carne de vaca", 1), nil, nil)
			@lxc_cordero = Node.new(Alimento.new([20,10,20,20,185], "carne de cordero", 1), nil, nil)
			@lxc_pollo = Node.new(Alimento.new([10,10,4,5.7,7.1], "pollo", 1), nil, nil)
